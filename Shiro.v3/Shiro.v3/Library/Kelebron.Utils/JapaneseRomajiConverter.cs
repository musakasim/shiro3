...
//https://github.com/tfreedman/JLPT/
...
namespace JLPT
{
    ...
    {
        private static string[] romajiHiraganaPairs =
        {
            "あ", "a", "い", "i", "う", "u", "え", "e", "お", "o", "か", "ka", "き", "ki", "く", "ku", "け", "ke", "こ", "ko", "さ",
            "sa",
            "し", "shi", "す", "su", "せ", "se", "そ", "so", "た", "ta", "ち", "chi", "つ", "tsu", "て", "te", "と", "to", "な",
            "na",
        ...
        private static string[] romajiHiraganaDigraphs =
        {
            "きゃ", "kya", "きゅ", "kyu", "きょ", "kyo", "しゃ", "sha", "しゅ", "shu", "しょ", "sho", "ちゃ", "cha", "ちゅ", "chu", "ちょ",
            "cho",
            "にゃ", "nya", "にゅ", "nyu", "にょ", "nyo", "ひゃ", "hya", "ひゅ", "hyu", "ひょ", "hyo", "みゃ", "mya", "みゅ", "myu", "みょ",
            "myo",
            "りゃ", "rya", "りゅ", "ryu", "りょ", "ryo", "ぎゃ", "gya", "ぎゅ", "gyu", "ぎょ", "gyo", "じゃ", "ja", "じゅ", "ju", "じょ",
            "jo",
            "ぢゃ", "ja", "ぢゅ", "ju", "ぢょ", "jo", "びゃ", "bya", "びゅ", "byu", "びょ", "byo", "ぴゃ", "pya", "ぴゅ", "pyu", "ぴょ",
            "pyo"
        };

        private static readonly string[] romajiKatakanaPairs =
        {
            "ア", "a", "イ", "i", "ウ", "u", "エ", "e", "オ", "o", "カ", "ka", "キ", "ki", "ク", "ku", "ケ", "ke", "コ", "ko", "サ",
            "sa", "シ", "shi",
            "ス", "su", "セ", "se", "ソ", "so", "タ", "ta", "チ", "chi", "ツ", "tsu", "テ", "te", "ト", "to", "ナ", "na", "ニ",
            "ni", "ヌ", "nu", "ネ", "ne",
            "ノ", "no", "ハ", "ha", "ヒ", "hi", "フ", "fu", "ヘ", "he", "ホ", "ho", "マ", "ma", "ミ", "mi", "ム", "mu", "メ", "me",
            "モ", "mo", "ヤ", "ya",
            "ユ", "yu", "ヨ", "yo", "ラ", "ra", "リ", "ri", "ル", "ru", "レ", "re", "ロ", "ro", "ワ", "wa", "ヲ", "wo", "ン", "n",
            "ガ", "ga", "ギ", "gi",
            "グ", "gu", "ゲ", "ge", "ゴ", "go", "ザ", "za", "ジ", "ji", "ズ", "zu", "ゼ", "ze", "ゾ", "zo", "ダ", "da", "デ", "de",
            "ド", "do",
            "バ", "ba", "ビ", "bi", "ブ", "bu", "ベ", "be", "ボ", "bo", "パ", "pa", "ピ", "pi", "プ", "pu", "ペ", "pe", "ポ", "po",
            "ヴ", "vu"
        };

        private static readonly string[] romajiKatakanaDigraphs =
        {
        ...
        public static string KanaToRomaji(String kana)
        {
            ...
            {
                kana = JapaneseKanaConverter.HiraganaToKatakana(kana);
            }
            String converted = "";
            for (int i = 0; i < kana.Length; i++)
            {
                ...
                {
                    converted += JapaneseKanaConverter.HiraganaToKatakana(Convert.ToString(kana[i]));
                }
                else
                {
                    converted += kana[i];
            ...
            var digraph = new Dictionary<string, string>();
            ...
            for (int i = 0; i < kana.Length; i++)
            {
                ...
                {
                    string buffer = "";
                    if (((i + 1) < kana.Length) && dictionary.ContainsKey(Convert.ToString(kana[i + 1])) &&
                        JapaneseKanaClassifier.IsJapaneseScript(Convert.ToString(kana[i + 1])))
                    {
                        buffer += dictionary[Convert.ToString(kana[i + 1])];
                        modified += buffer[0];
                    }
                    else if ((i + 1) == kana.Length ||
                             (((i + 1) < kana.Length) && dictionary.ContainsKey(Convert.ToString(kana[i + 1])) &&
                              !JapaneseKanaClassifier.IsJapaneseScript(Convert.ToString(kana[i + 1]))))
                        modified += "-";
                }
                else if (kana[i] == 'ー')
